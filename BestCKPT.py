...
#Original GITHUB location   https://github.com/bluecamel/best_checkpoint_copier
import glob
import os
import shutil
...
class BestCheckpointCopier(tf.estimator.Exporter):
  ...
  def _copyCheckpoint(self, checkpoint):
    ...
    for file in glob.glob(r'{}*'.format(checkpoint.path)):
      ...
      checkPointNumberSt=re.findall(r'\d+', os.path.basename(file))[0]
      newFolderPath=os.path.join(desination_dir, checkPointNumberSt+'_ValidAccuracy_'+str(round(checkpoint.score,2)))
      if(os.path.isdir(newFolderPath)==False):
         shutil.os.makedirs(newFolderPath, exist_ok=True)
